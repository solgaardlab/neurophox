...
class MultiModelRunner:
    ...
    Args:
        ...
        cost_fn: cost function used to evaluate things
    """
    ...
    def __init__(self, experiment_name: str, layer_names: List[str],
                 ...
                 store_params: bool = True, cost_fn: Callable = complex_mse):  # e.g., logdir=/data/tensorboard/neurophox/
    ...
    def iterate(self, targets: np.ndarray, grad_method: Optional[Callable] = None, **grad_kwargs):
        """
        Run gradient update toward a target unitary :math:`U`.

        Args:
            targets: Target unitary, :math:`U`.
            grad_method: a method for calculating the gradient (defaults to automatic differentiation)
            grad_kwargs: keyword arguments for `grad_method`

        """
        if self.train_on_test:
            x_train, y_train = tf.eye(self.layers[0].units, dtype=TF_COMPLEX), \
                               tf.convert_to_tensor(targets, dtype=TF_COMPLEX)
        else:
            x_train, y_train = generate_keras_batch(self.layers[0].units, targets, self.batch_size)
        for name, layer, optimizer in zip(self.layer_names, self.layers, self.optimizers):
            if not grad_method:
                ...
                # weight update already completed in the function call
                grads = grad_method(layer, self.cost_fn, x_train, y_train, **grad_kwargs)
            ...
            self.losses[name].append(tf.reduce_sum(
                complex_mse(layer(tf.eye(layer.units, dtype=TF_COMPLEX)),
                            tf.convert_to_tensor(targets.astype(NP_COMPLEX)))).numpy()
                                     )
            ...
            if self.iters % self.iterations_per_epoch == 0 and self.store_params:
                if not isinstance(layer, SVD):
                    ...
                    self.results[name].append({
                        ...
                        "error_mag": np.abs(estimate - targets)
                    })
    ...
    def run(self, num_epochs: int, target: np.ndarray, pbar: Optional[Callable] = None):
        """

        Args:
            num_epochs: Number of epochs (defined in terms of `iterations_per_epoch`)
            target: Target unitary, :math:`U`.
        ...
        for _ in iterator:
            self.iterate(target)
            # learning_rate *= decay_rate = 0.999
